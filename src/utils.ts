...
export function formatQuestionValue(
	...
	return unsafeHTML(
		value.replaceAll(
			'%s',
			`<span class="opacity-70">${placeHolderTransform}</span>`,
			// `<span class="rounded-full bg-(--md-sys-color-on-surface) size-4 inline-block align-middle mx-1"></span>`,
			// `<span class="text-(--md-sys-color-outline-variant)">${placeHolderTransform}</span>`,
			// `<md-suggestion-chip>${placeHolderTransform}</md-suggestion-chip>`,
		),
	)
}
